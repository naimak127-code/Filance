...
const [showAccounts, setShowAccounts] = useState(false);
  ...
  const expenseThisMonth = data.transactions
    .filter(t => t.type === "expense")
    .reduce((s, t) => s + t.amount, 0);

  const balanceOverall = incomeThisMonth - expenseThisMonth;

  /* ✅ DONUT DATA */
  const accountDonutData = data.accounts.reduce((acc, a) => {
    acc[a.name] = a.balance;
    return acc;
  }, {});

  const expenseDonutData = data.transactions
    .filter(t => t.type === "expense")
    .reduce((acc, t) => {
